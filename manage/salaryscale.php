...
if ($Mode=='ADD_ITEM' || $Mode=='UPDATE_ITEM') {
	...
	// }
		...
		write_scale($_POST['selected_id'], $_POST['name'], $_POST['payBasis'], $_POST['gl_debit'], $_POST['gl_credit'],$_POST[''],$_POST['']);
...
$th = array(_("Id"), _("Name"), _('Pay basis'), _('Debit GL'), _('Credit GL'), _('Edit'), _('Remove'));
...
while ($myrow = db_fetch($result)) {
	...
	label_cell($myrow["gl_debit"]);
	label_cell($myrow["gl_credit"]);
...
if($selected_id != -1) {
 	...
 	if ($Mode == 'Edit') {
		...
		$salary_scale_result = db_query(get_salary_scale($selected_id));
		while($myrow = db_fetch($salary_scale_result))
		{
			display_notification($myrow["scale_name"]);
...
//text_row_ex(_('Id number').':','name',37,50);
text_row_ex(_('Salary Scale Name').':', 'name', 37, 50);
//text_row(_("Salary Scale Name :"), 'name', null, 37, 50);
//gl_all_accounts_list_row(_('Debit GL'), 'AccountId');
...
gl_all_accounts_list_row(_('Debit GL'), 'gl_debit');
gl_all_accounts_list_row(_('Credit GL'), 'gl_credit');
end_table(1);
